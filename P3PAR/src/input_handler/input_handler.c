...
void i_read_elements_from_file(Input *input){
   ...
   int r;
    char value[MAX_LEN], category[MAX_LEN];
    while ((r = fscanf(input->input_file, " < %[^,] , %[^>] > ", value, category)) == 2) { //%[] means read everythin and ^ means NOT so %[^,] means read everything except ,

        // printf("Value: %s | Category: %s\n", value, category);
        Token token;
        token = t_token_create();
        t_token_append_char(token, value);
        t_token_udpate_category(token, category);
